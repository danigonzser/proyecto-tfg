...
\textit{Jest} es un framework de testing JavaScript creado por Facebook. Usualmente, se utiliza para los tests unitarios. Es compatible con TypeScript a través del paquete \textit{ts-jest}. \textit{Jest} destaca por su facilidad de uso y rapidez. Además, incluye funcionalidades como partir desde una configuración nula o casi nula, las snapshots que captura el estado de un objeto y lo expone para que sirva de referencia para debugear, los tests se realizan de forma paralela para maximizar la eficiencia. Además, incorpora una buena compatibilidad con muchos de los lenguajes, frameworks y librerías más usadas.
